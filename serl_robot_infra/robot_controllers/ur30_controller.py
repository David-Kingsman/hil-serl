...
class UrImpedanceController(threading.Thread):
    ...
    async def _update_robot_state(self):
        ...
        position_normalized = (position - self.robotiq_gripper.get_min_position()) / (self.robotiq_gripper.get_max_position() - self.robotiq_gripper.get_min_position())
    ...
    async def send_gripper_command(self, force_release=False):
        if force_release:
            ...
            await self.robotiq_gripper.move(self.robotiq_gripper.get_open_position(), 64, 50)
        ...
        if self.target_grip[0] > 0.5 and timeout_exceeded and self.gripper_state[1] < 0.5:
            ...
            await self.robotiq_gripper.move(self.robotiq_gripper.get_closed_position(), 64, 50)
            self.target_grip[0] = 0.0
