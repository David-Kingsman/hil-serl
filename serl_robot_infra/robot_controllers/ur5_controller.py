...
class UrImpedanceController(threading.Thread):
    def __init__(
        ...
        self.robotiq_gripper: Gripper = None
    ...
    async def start_ur_interfaces(self, gripper=True):
        ...
        if gripper:
            ...
            self.robotiq_gripper = Gripper(self.robot_ip)
    ...
    async def _update_robot_state(self):
        ...
        # For two-finger gripper: use position and OBJ status
        position = await self.robotiq_gripper.get_current_position()
        obj_status = await self.robotiq_gripper._get_var(self.robotiq_gripper.OBJ)
        min_pos = self.robotiq_gripper.get_open_position()
        max_pos = self.robotiq_gripper.get_closed_position()
    ...
    async def send_gripper_command(self, force_release=False):
        if force_release:
            ...
            # open fully
            await self.robotiq_gripper.move_and_wait_for_pos(self.robotiq_gripper.get_open_position(), 64, 1)
        ...
        if self.target_grip[0] > 0.5 and timeout_exceeded and self.gripper_state[1] < 0.5:
            ...
            await self.robotiq_gripper.move_and_wait_for_pos(self.robotiq_gripper.get_closed_position(), 64, 1)
            self.target_grip[0] = 0.0
